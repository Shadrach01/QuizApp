...
import com.example.quizapp.localRepository.ArtAndCraftDataSource
...
class ArtAndCraftViewModel : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            // Questions
            val questions = ArtAndCraftDataSource.questions
            ...
            is QuestionEvent.NextQuestion -> {
                if (_currQuestion.value.isEnabled) {
                    ...
                    if (_currQuestion.value.currQuestion?.correctAnswer == _currQuestion.value.currAns) {
                        ...
                        Log.d(
                            "Debug",
                            "Correct Answer: ${_currQuestion.value.currQuestion?.correctAnswer}"
                        )
                        Log.i(tag, "Selected Answer: ${_currQuestion.value.currAns}")

                    } else {
                        event.navHostController.navigate(QuizAppTitle.GameOverScreen.name)
                        Log.i(
                            tag,
                            "Wrong Answer: ${_currQuestion.value.currQuestion?.correctAnswer}, Selected: ${_currQuestion.value.currAns}"
                        )
                        _currQuestion.value = currQuestion.value.copy(
                            questionNo = 1,
                            isEnabled = true,
